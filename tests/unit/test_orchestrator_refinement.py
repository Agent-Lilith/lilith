...
from src.contracts.mcp_search_v1 import SearchResultV1, RetrievalMethod
...
class TestOrchestratorRefinement:
    ...
    def test_should_refine_triggers(self, orchestrator):
        ...
        decisions_filtered = [RoutingDecision("test", ["vector"], "query", [{"field": "date"}])]
        should, reason = orchestrator._should_refine(results, intent, decisions_filtered)
        ...
        should, reason = orchestrator._should_refine(low_conf_results, intent, decisions)
        assert should is True
    ...
    async def test_refine_actions(self, orchestrator):
        ...
        orchestrator._capabilities.can_handle.side_effect = lambda src, meth: meth in ["fulltext", "vector"]
        
        decisions = [
            RoutingDecision("sourceA", ["structured"], "query", [])
        ]
