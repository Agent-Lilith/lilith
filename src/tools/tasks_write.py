    ...
    def _sync_execute(
        ...
        try:
            if action == "create":
                ...
                if due:
                    body["due"] = _parse_due(due)
                task = (
                    self._service.tasks.tasks()
                    .insert(tasklist=lid, body=body)
                    .execute()
                )
                tid = task.get("id", "")
            ...
            if action == "update":
                ...
                if due:
                    update_body["due"] = _parse_due(due)
                if status and status.lower() in ("needsaction", "completed"):
                    update_body["status"] = status.lower()
                if not update_body:
                    return ToolResult.fail(
                        "update requires at least one field (title, notes, due, status)."
                    )
                ...
                summary_msg = (
                    f"Update task '{summary_title}' with the requested changes?"
                )
            ...
            if action == "update_list":
                if not list_id:
                    return ToolResult.fail("update_list requires list_id.")
                if not title:
                    return ToolResult.fail("update_list requires title.")
                existing = (
                    self._service.tasks.tasklists()
                    .get(tasklist=list_id.strip())
                    .execute()
                )
                ...
                summary_msg = (
                    f"Rename task list to '{title.strip()}' (was '{summary_title}')?"
                )
            ...
            if action == "delete_list":
                if not list_id:
                    return ToolResult.fail("delete_list requires list_id.")
                existing = (
                    self._service.tasks.tasklists()
                    .get(tasklist=list_id.strip())
                    .execute()
    ...
    def execute_pending(self, pending_id: str) -> ToolResult:
        ...
        try:
            if action == "update":
                ...
                self._service.tasks.tasks().delete(
                    tasklist=payload["list_id"], task=payload["task_id"]
                ).execute()
                out = "Task deleted."
                ...
                lst = (
                    self._service.tasks.tasklists()
                    .patch(
                        tasklist=payload["list_id"], body={"title": payload["title"]}
                    )
                    .execute()
                )
                out = f"Task list renamed to: {lst.get('title', payload['title'])}"
            elif action == "delete_list":
                self._service.tasks.tasklists().delete(
                    tasklist=payload["list_id"]
                ).execute()
