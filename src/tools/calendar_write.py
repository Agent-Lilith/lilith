...
def _build_event_body(
    ...
    if visibility and visibility.lower() in ("default", "public", "private", "confidential"):
...
class CalendarWriteTool(Tool):
    ...
    async def execute(
        ...
        all_args = {
            "action": action, "event_id": event_id, "calendar_id": calendar_id,
            "title": title, "start": start, "end": end, "description": description,
            "location": location, "reminders": reminders, "recurrence": recurrence,
            "visibility": visibility, "attendees": attendees, "color_id": color_id
        }
        log_args = {k: v for k, v in all_args.items() if v}
        logger.tool_execute(self.name, log_args)

        return await asyncio.to_thread(
            self._sync_execute, action, event_id, calendar_id, title, start, end,
            description, location, reminders, recurrence, visibility, attendees, color_id
        )
    ...
    def _sync_execute(
        ...
        if not self._service.is_connected:
            return ToolResult.fail("Calendar not connected. User should run: python -m src.main google-auth")
        ...
        try:
            if action == "create":
                ...
                body = _build_event_body(
                    ...
                    reminders=reminders, recurrence=recurrence, visibility=visibility,
                    attendees=attendees, color_id=color_id,
                )
                event = self._service.calendar.events().insert(calendarId=cal_id, body=body).execute()
                eid = event.get("id", "")
            ...
            if action == "update":
                ...
                if not body:
                    return ToolResult.fail("update requires at least one field to change.")
                
                existing = self._service.calendar.events().get(calendarId=cal_id, eventId=event_id.strip()).execute()
                summary_title = (existing or {}).get("summary", "event")
                when = _format_event_when(existing or {})
                pending_id = str(uuid.uuid4())
                self._pending[pending_id] = {
                    "action": "update", "calendar_id": cal_id, "event_id": event_id.strip(), "body": body,
                }
                summary_msg = f"Update the appointment on {when} called '{summary_title}' with the requested changes?" if when else f"Update '{summary_title}' with the requested changes?"
            ...
            if action == "delete":
                ...
                pending_id = str(uuid.uuid4())
                self._pending[pending_id] = {
                    "action": "delete", "calendar_id": cal_id, "event_id": event_id.strip(), "body": None,
                }
                summary_msg = f"Delete the appointment on {when} called '{summary_title}'?" if when else f"Delete '{summary_title}'?"
                out = f"I need your confirmation to delete this event. {format_confirm_required(self.name, pending_id, summary_msg)}"
                return ToolResult.ok(out)

            return ToolResult.fail(f"Unknown action: {action}. Use create, update, or delete.")
    ...
    def execute_pending(self, pending_id: str) -> ToolResult:
        if pending_id not in self._pending:
            return ToolResult.fail("Confirmation expired or invalid. Please try the action again.")
        ...
        try:
            if action == "update":
                body = payload["body"]
                event = self._service.calendar.events().patch(calendarId=cal_id, eventId=event_id, body=body).execute()
                out = f"Event updated: {event.get('summary', event_id)}"
            else: # delete
                self._service.calendar.events().delete(calendarId=cal_id, eventId=event_id).execute()
                out = "Event deleted."
