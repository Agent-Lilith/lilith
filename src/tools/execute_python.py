...
class ExecutePythonTool(Tool):
    ...
    def _sync_execute(self, code: str) -> ToolResult:
        ...
        self._container.put_archive("/workspace", self._create_tar_with_code(wrapped_code))
        ...
        error_reason = None if success else (combined_output or "Unknown error in sandbox")
        logger.tool_result(self.name, len(combined_output), success, error_reason=error_reason or None)
        if success:
