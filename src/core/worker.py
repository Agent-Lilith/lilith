...
current_llm_client: contextvars.ContextVar[Optional[Any]] = contextvars.ContextVar("current_llm_client", default=None)
...
class Worker:
    ...
    async def process(
        ...
        logger.debug(f"✅ Worker task completed. Input: {len(data)} chars -> Output: {len(result)} chars")
        return result
